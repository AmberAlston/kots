...
func EnsurePermissionsCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			kotsadmNamespace := v.GetString("kotsadm-namespace")
			if err := validateKotsadmNamespace(kotsadmNamespace); err != nil {
				return err
			}
	...
	cmd.Flags().String("kotsadm-namespace", "", "namespace in which kots/kotsadm is installed")
	cmd.Flags().String("velero-namespace", "", "namespace in which velero is installed")

	return cmd
}

func validateKotsadmNamespace(namespace string) error {
	if namespace == "" {
		return errors.New("kotsadm-namespace is required")
	}
	if strings.Contains(namespace, "_") {
		return errors.New("kotsadm-namespace should not contain the _ character")
	}
